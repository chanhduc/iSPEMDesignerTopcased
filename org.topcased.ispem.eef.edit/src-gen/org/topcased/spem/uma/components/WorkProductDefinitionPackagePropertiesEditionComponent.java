...
import org.topcased.spem.uma.parts.WorkProductDefinitionPackagePropertiesEditionPart;
...
public class WorkProductDefinitionPackagePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (isAccessible(UmaViewsRepository.WorkProductDefinitionPackage.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, workProductDefinitionPackage.getName()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmaViewsRepository.WorkProductDefinitionPackage.Properties.name == event.getAffectedEditor()) {
			workProductDefinitionPackage.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (SpemPackage.eINSTANCE.getMethodContentPackageableElement_Name().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(UmaViewsRepository.WorkProductDefinitionPackage.Properties.name)) {
