...
import org.topcased.spem.parts.MethodPluginPropertiesEditionPart;
...
public class MethodPluginPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (isAccessible(SpemViewsRepository.MethodPlugin.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, methodPlugin.getName()));
			...
			if (isAccessible(SpemViewsRepository.MethodPlugin.Properties.basePlugin)) {
				basePart.addFilterToBasePlugin(new EObjectFilter(SpemPackage.Literals.METHOD_PLUGIN));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (SpemViewsRepository.MethodPlugin.Properties.name == event.getAffectedEditor()) {
			methodPlugin.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (SpemPackage.eINSTANCE.getMethodLibraryPackageableElement_Name().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(SpemViewsRepository.MethodPlugin.Properties.name)) {
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			SpemPackage.eINSTANCE.getMethodPlugin_OwnedMethodContentPackage(),
			SpemPackage.eINSTANCE.getMethodPlugin_OwnedProcessPackage(),
			SpemPackage.eINSTANCE.getMethodPlugin_BasePlugin()		);
