...
public class MethodConfigurationPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (isAccessible(SpemViewsRepository.MethodConfiguration.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, methodConfiguration.getName()));
			...
			if (isAccessible(SpemViewsRepository.MethodConfiguration.Properties.baseConfiguration)) {
				basePart.addFilterToBaseConfiguration(new EObjectFilter(SpemPackage.Literals.METHOD_CONFIGURATION));
				// Start of user code for additional businessfilters for baseConfiguration
				// End of user code
			}
			if (isAccessible(SpemViewsRepository.MethodConfiguration.Properties.processPackageSelection)) {
				basePart.addFilterToProcessPackageSelection(new EObjectFilter(SpemPackage.Literals.PROCESS_PACKAGE));
				// Start of user code for additional businessfilters for processPackageSelection
				// End of user code
			}
			if (isAccessible(SpemViewsRepository.MethodConfiguration.Properties.contentPackageSelection)) {
				basePart.addFilterToContentPackageSelection(new EObjectFilter(SpemPackage.Literals.METHOD_CONTENT_PACKAGE));
				// Start of user code for additional businessfilters for contentPackageSelection
				// End of user code
			}
			if (isAccessible(SpemViewsRepository.MethodConfiguration.Properties.substractedCategory)) {
				basePart.addFilterToSubstractedCategory(new EObjectFilter(SpemPackage.Literals.CATEGORY));
			...
			if (isAccessible(SpemViewsRepository.MethodConfiguration.Properties.processView)) {
				basePart.addFilterToProcessView(new EObjectFilter(SpemPackage.Literals.CATEGORY));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (SpemViewsRepository.MethodConfiguration.Properties.name == event.getAffectedEditor()) {
			methodConfiguration.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (SpemPackage.eINSTANCE.getMethodLibraryPackageableElement_Name().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(SpemViewsRepository.MethodConfiguration.Properties.name)) {
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			SpemPackage.eINSTANCE.getMethodLibraryPackageableElement_Name(),
			SpemPackage.eINSTANCE.getMethodConfiguration_BaseConfiguration(),
			SpemPackage.eINSTANCE.getMethodConfiguration_ProcessPackageSelection(),
			SpemPackage.eINSTANCE.getMethodConfiguration_ContentPackageSelection(),
			SpemPackage.eINSTANCE.getMethodConfiguration_SubstractedCategory(),
			SpemPackage.eINSTANCE.getMethodConfiguration_DefaultView(),
			SpemPackage.eINSTANCE.getMethodConfiguration_ProcessView()		);
			...
			try {
				if (SpemViewsRepository.MethodConfiguration.Properties.name == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(SpemPackage.eINSTANCE.getMethodLibraryPackageableElement_Name().getEAttributeType(), (String)newValue);
					}
