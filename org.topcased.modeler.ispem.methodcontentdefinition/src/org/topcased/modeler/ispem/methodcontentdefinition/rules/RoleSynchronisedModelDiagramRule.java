...
public class RoleSynchronisedModelDiagramRule extends SynchronizedModelDiagramRule {
	/**
    ...
    public boolean hasRightGraphicParent(IDecoratorTarget pTarget)
    {
        ...
        if (lEditPart != null)
        {
            if (lEditPart.getViewer() instanceof ModelerGraphicalViewer)
            {
                ...
                {
                    //In case that RoleDefinition is contained in RoleDefinitionPackage 
                	if (lSemanticParent instanceof RoleDefinitionPackage )
                	{
                		return lSemanticParent.eContainer().equals(lGraphicalParent);
                	}
                	//Another case:RoleDefinition is contained in MethodContentPackage 
