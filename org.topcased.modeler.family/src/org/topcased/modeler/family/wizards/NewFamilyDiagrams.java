 ...
 ******************************************************************************/
package org.topcased.modeler.family.wizards;
...
import org.topcased.modeler.family.FamilyImageRegistry;
 ...
 * @generated
 */
public class NewFamilyDiagrams extends Wizard implements INewWizard {
	/**
	 * @generated
	 */
	...
	public void init(IWorkbench workbench, IStructuredSelection sel) {
		...
		setDefaultPageImageDescriptor(FamilyImageRegistry
				.getImageDescriptor("NEW_PAGE_WZD"));
		setDialogSettings(FamilyPlugin.getDefault().getDialogSettings());
		setWindowTitle("Create new Family diagrams");
	}
		...
		if (diagPage.isPageComplete()) {
			if (diagPage.isNewModel()) {
				...
				if (createdFile != null && result) {
					...
					try {
						IDE.openEditor(FamilyPlugin.getDefault().getWorkbench()
								.getActiveWorkbenchWindow().getActivePage(),
								createdFile, true);
					} catch (PartInitException pie) {
						FamilyPlugin.log(pie);
					}
				}
			} else {
	...
	public void addPages() {
		diagPage = new FamilyDiagramsPage("New Family Diagram", selection);
		diagPage.setTitle("Family Diagrams");
	...
	private boolean createModelFile() {
		try {
			...
			FamilyPlugin.log(ce);
			FamilyPlugin.displayDialog(null,
